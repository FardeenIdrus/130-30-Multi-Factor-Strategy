...
class TestFetchSingleFundamental:
    ...
    def test_simfin_source(self, fetcher):
        ...
        with patch.object(fetcher, "_fetch_simfin_fundamentals", return_value=fake_df):
            result = fetcher._fetch_single_fundamental("AAPL", period="5y", source="simfin")
    ...
    def test_waterfall_source_delegates(self, fetcher):
        ...
        with patch.object(fetcher, "_fetch_waterfall_fundamentals", return_value=fake_df) as mock_wf:
            result = fetcher._fetch_single_fundamental("AAPL", period="5y", source="waterfall")
...
class TestEdgarGetJson:
    ...
    @patch("modules.input.data_collector.requests.get")
...
class TestResolveCik:
    ...
    def test_resolves_known_ticker(self, fetcher):
        with patch.object(fetcher, "_edgar_get_json", return_value={
...
class TestEdgarFetchConcept:
    ...
    def test_parses_concept_data(self, fetcher):
        payload = {
            "units": {
                "USD": [
                    {"end": "2024-03-30", "start": "2024-01-01", "val": 3.5e11,
                     "filed": "2024-05-01", "form": "10-Q"},
                    {"end": "2024-06-30", "start": "2024-04-01", "val": 3.6e11,
                     "filed": "2024-08-01", "form": "10-Q"},
                ]
            }
        }
    ...
    def test_filters_by_cutoff(self, fetcher):
        payload = {
            "units": {
                "USD": [
                    {"end": "2020-03-30", "val": 1e11, "filed": "2020-05-01", "form": "10-Q"},
                    {"end": "2024-03-30", "val": 3.5e11, "filed": "2024-05-01", "form": "10-Q"},
                ]
            }
        }
        with patch.object(fetcher, "_edgar_get_json", return_value=payload):
...
class TestFetchEdgarFundamentals:
    ...
    def test_happy_path(self, fetcher):
        ...
        concept_df = pd.DataFrame({
            "end": ["2024-03-30"], "start": ["2024-01-01"],
            "val": [3.5e11], "filed": [pd.Timestamp("2024-05-01")],
        })
        empty_concept = pd.DataFrame(columns=["end", "start", "val", "filed"])

        with patch.object(fetcher, "_resolve_cik", return_value="0000320193"), \
             patch.object(fetcher, "_edgar_get_fiscal_periods", return_value=periods), \
             patch.object(fetcher, "_edgar_fetch_concept", side_effect=[
    ...
    def test_debt_fallback_to_tier2(self, fetcher):
        """LongTermDebt is empty; should fall back to LongTermDebtAndCapitalLeaseObligations."""
        periods = pd.DataFrame({
            "report_date_str": ["2024-03-30"],
            "report_date": [pd.Timestamp("2024-03-30")],
            "fiscal_year": [2024],
        ...
        val_df = pd.DataFrame({
            "end": ["2024-03-30"], "start": ["2024-01-01"],
            "val": [1e11], "filed": [pd.Timestamp("2024-05-01")],
        })
        ...
        with patch.object(fetcher, "_resolve_cik", return_value="0000320193"), \
             ...
             patch.object(fetcher, "_edgar_fetch_concept", side_effect=[
                 ...
                 empty, empty,    # shares
                 empty, empty,    # EPS diluted, EPS basic
    ...
    def test_ytd_to_standalone_conversion(self, fetcher):
        ...
        periods = pd.DataFrame({
            ...
            "report_date": pd.to_datetime(["2024-03-30", "2024-06-30", "2024-09-30"]),
        ...
        with patch.object(fetcher, "_resolve_cik", return_value="0000320193"), \
             ...
             patch.object(fetcher, "_edgar_fetch_concept", side_effect=[
                 ...
                 empty, empty,     # Equity primary, Equity fallback
                 empty, empty,     # shares (2 tags)
                 empty, empty,     # EPS diluted, EPS basic
                 empty,            # LongTermDebt (empty → tier 2)
                 empty,            # LongTermDebtAndCapitalLeaseObligations (empty → tier 3)
                 empty, empty,     # LongTermDebtNoncurrent, LongTermDebtCurrent
             ]):
            result = fetcher._fetch_edgar_fundamentals("AAPL")
...
class TestFetchWaterfallFundamentals:
    ...
    def test_uses_all_sources(self, fetcher):
        ...
        with patch.object(fetcher, "_fetch_edgar_fundamentals", return_value=edgar_df), \
             patch.object(fetcher, "_fetch_simfin_fundamentals", return_value=simfin_df), \
             patch.object(fetcher, "_fetch_yfinance_fundamentals", return_value=yf_df):
    ...
    def test_all_sources_empty(self, fetcher):
        with patch.object(fetcher, "_fetch_edgar_fundamentals", return_value=pd.DataFrame()), \
             patch.object(fetcher, "_fetch_simfin_fundamentals", return_value=pd.DataFrame()), \
             patch.object(fetcher, "_fetch_yfinance_fundamentals", return_value=pd.DataFrame()):
            result = fetcher._fetch_waterfall_fundamentals("AAPL", "5y")
